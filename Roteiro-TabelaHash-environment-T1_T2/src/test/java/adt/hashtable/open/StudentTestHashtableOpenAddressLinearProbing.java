...
public class StudentTestHashtableOpenAddressLinearProbing {
	...
	public void testSize() {
		assertEquals(4,table1.size());
		table1 = new HashtableOpenAddressLinearProbingImpl<HashtableElement>(1000000, HashFunctionClosedAddressMethod.MULTIPLICATION);
		for(int i = 1, j = 1000000; i <= 500000;i++, j--){
			table1.insert(new HashtableElement(i));
			table1.insert(new HashtableElement(j));
			assertEquals(i*2, table1.size());
		}
		assertEquals(1000000, table1.size());
		
		for(int i = 1, j = 1000000; i <= 500000;i++, j--){
			assertTrue(table1.search(new HashtableElement(i)) != null);
			assertTrue(table1.search(new HashtableElement(j)) != null);
		}
	}
}
